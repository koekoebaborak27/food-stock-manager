...
> **マージ後、本番へどう届くか**（自動デプロイ・DBマイグレーションの要否を含む全体の流れ）は、本番リリース手順をまとめた文書を `docs/development/` に作って参照してください（テンプレートには入っていません）。
...
- **`paths-ignore`が効くのはGitHub Actionsだけです。** `main`へのpushを契機に自動デプロイする仕組み（クラウド側のビルドトリガーなど）を別に持つ場合、ドキュメントだけの変更でもデプロイが走ります。止めたい場合は、その仕組み側のファイルフィルタで除外するか、コミットメッセージに`[skip ci]` / `[ci skip]`を付けます（多くのサービスが同じ文字列に対応しています）。**自動デプロイを設定したら、`main`へのpushが本番へ反映されることをここに追記してください。**
