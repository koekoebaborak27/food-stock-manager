# おうちde常備食 — エージェント向け方針（正本）

家庭で作った作り置き料理と、冷蔵・冷凍・常温の食品ストックを家族で共有するスマホ向け PWA。
家族グループ（世帯）単位で常備食と買い物リストを共有し、食べ忘れと重複購入を減らすことを目的とする。
要件の正本は `docs/specs/01_requirements/`。
